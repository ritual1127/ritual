...
const files=[];
function walk(d){for(const x of fs.readdirSync(d,{withFileTypes:true})){if(['.git','dist','node_modules','feedback-api'].includes(x.name))continue;const p=path.join(d,x.name);if(x.isDirectory())walk(p);else if(x.name==='index.html')files.push(p)}}walk(root);
